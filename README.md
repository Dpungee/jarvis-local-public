...
5. Double-click `start_jarvis_presence.bat` for the recommended browser interface,
   ...
   terminal chat. See the [Presence guide](docs/PRESENCE_UI.md) and the
...
- A foreground operator can store one project-scoped fact without a model or tool call by sending `Remember this project fact: {"subject":"...","predicate":"...","value":"..."}` as a standalone turn. The exact parser, project binding, fixed provenance, supersession, and non-echoing receipt are deterministic and fail closed. See [Governed project memory](docs/GOVERNED_PROJECT_MEMORY.md) for syntax, scope, threat boundaries, model compatibility, and current limits.
