...
which also documents the sealed holdouts, their runtime pins, and how to reseal
them after a legitimate change to a pinned source file.
...
- `/memory` shows recently stored non-claim durable memories. Structured claims
  use scoped typed retrieval and their raw backing rows are never shown by this
  unscoped listing command.
...
- With `JARVIS_MEMORY_AUTO_IMPROVE=true`, every memory actually injected into a task is linked to that task's measured outcome. A conservative Beta-prior utility score then reranks only close future matches. This internal evidence update requires no approval, never changes tools or policy, never deletes the append-only retrieval ledger on its own (only an operator's `Erase this project fact:` removes a fact's retrieval rows, with a tombstone receipt), and cannot overpower strong relevance.
- `JARVIS_STRATEGY_TRANSFER=observe` records a closed set of procedural strategies only when exact runtime receipts prove them. Cross-domain candidates must remain in the same project, come from a different calibrated task family, retain valid lesson provenance, and be fresh and uncontradicted. Observe mode never changes model context. `trial` is an explicit operator-only evaluation mode: it additionally requires `jarvis strategy-transfer start` with one project, one to three closed task families, a 40-200 sample cap divisible by four, a maximum 14-day lifetime, and one or more closed strategy labels. Jarvis derives the exact installed fixture/evaluator/config/runtime pins itself and generates a cryptographically random assignment seed; the normal operator never copies hashes or sees the seed. The manifest accepts no task prose, notes, paths, URLs, or personal identifiers. `advise` is unavailable unless that pre-outcome control/treatment trial produces a valid pinned causal attestation and the operator separately runs `jarvis strategy-transfer promote ID`. Setting the mode alone cannot activate advice. Any advisory remains labels-only, never lesson prose, and cannot change tools, approvals, policy, scope, routing, or verification. `jarvis strategy-transfer status [--json]` is prompt-free, while `abort ID` permanently stops new assignments.
...
- A foreground operator can store one project-scoped fact without a model or tool call by sending `Remember this project fact: {"subject":"...","predicate":"...","value":"..."}` as a standalone turn. The exact parser, project binding, fixed provenance, supersession, and non-echoing receipt are deterministic and fail closed. `Forget this project fact: {"subject":"...","predicate":"..."}` retires one, `Erase this project fact:` deletes every version with a tombstone receipt, and `python -m jarvis facts` lists them. Every claim write, retraction, erasure, conversation deletion, and proposal receipt is an event on an append-only, keyed, hash-chained memory spine, and since schema 47 every ordinary memory and lesson row carries lineage and a digest-only receipt too (the model's `remember` tool is receipted as `model` with the gate that admitted it). `python -m jarvis spine verify | rebuild-claims [--apply --yes] | rebuild-memories | tail` verifies the chain and lineage, replays the spine into shadow projections, and reconciles the live claim rows in place only on an explicit `--apply --yes` (see [The memory spine](docs/MEMORY_SPINE.md)). At schema 48 a second derived projection, the temporal graph, answers questions that span two or three stored facts in either direction of every triple, deterministically and with no model call: `python -m jarvis graph status | verify | rebuild [--apply --yes] | paths "<subject>"` (see [The temporal graph](docs/MEMORY_GRAPH.md)). One ordinary memory can now be erased by its id with `Erase memory #<id>`, `python -m jarvis memory erase <id> --yes`, or the ids shown by `python -m jarvis memory list` and `/memory`. An ordinary turn that states a fact in update form receives a deterministic "Not stored" note with the exact command to paste (or reply `store it` to confirm exactly that), a question that spans two or three stored facts is answered from a bounded chain over the graph in either direction, a stored fact for a named subject outranks weak web intent such as "latest", past-tense questions see superseded values as history (a retired fact still answers them until it is erased), an unknown project-shaped subject gets an explicit not-recorded cue while world knowledge still answers, and `/facts` in chat lists the active facts. At schema 49 the learning loop gains a governed ladder: a verified outcome no longer writes a live learned skill in the same turn. A twice-reused lesson in a calibrated, non-regressed family is distilled into a **staged** document the model can neither see nor read through its file tools, and only an operator-typed `Approve skill promotion #N <confirmation code>` makes it live; `Roll back skill promotion #N` restores the exact prior bytes and needs no code. An append-only calibration ledger of mechanically cut epochs records what the gate was reading at each authorisation, so "competence did not regress" is a computable predicate that refuses a promotion rather than a claim. `python -m jarvis ladder status | list | show | stage | approve | rollback | discard | seal | verify | ledger` and `/ladder` are the operator surfaces (see [The learning ladder](docs/LEARNING_LADDER.md)). See [Governed project memory](docs/GOVERNED_PROJECT_MEMORY.md) for syntax, scope, threat boundaries, model compatibility, and current limits.
...
| `JARVIS_STRATEGY_TRANSFER` | `observe` | `disabled`, `observe`, `trial`, or `advise`; `trial` additionally requires an explicitly started bounded pinned manifest, while `advise` requires both a valid causal attestation and explicit operator promotion |
...
| `JARVIS_MEMORY_PROPOSER` | `assisted` | `rules` or `assisted`; whether the local model may propose a governed project-fact command (grounded verbatim in the operator's words, parser-validated, operator-confirmed) when the deterministic grammar cannot split a stated fact. No mode lets a model write memory |
